...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 max-w-2xl mx-auto overflow-hidden">
        ...
        <form method="POST" action="{{ route('menus.store') }}" class="p-8">
            ...
            <!-- Input: Icon -->
            <div class="mb-6">
                <label for="icon" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                    Ikon (Class CSS)
                </label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    ...
                    <input type="text" id="icon" name="icon" 
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 @error('icon') border-red-500 @enderror" 
                        placeholder="Contoh: fa fa-home atau heroicons" value="{{ old('icon') }}">
                </div>
                <p class="mt-1 text-xs text-gray-500">Masukkan nama class ikon (FontAwesome/Heroicons).</p>
                @error('icon')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
